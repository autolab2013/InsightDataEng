...
if __name__ == '__main__':
    ...
    single_count_tuples = pool.map(MyMapReduce.Map, partitioned_text)

    # Organize the count tuples; lists of tuples by token key
    token_to_tuples = MyMapReduce.Partition(single_count_tuples)

    # Collapse the lists of tuples into total term frequencies
    term_frequencies = pool.map(MyMapReduce.Reduce, token_to_tuples.items())
